    ...
    describe("updatePaymentStatus", () => {
        it("should update payment status to paid", async () => {
            ...
            expect(OrderModel.findByIdAndUpdate).toHaveBeenCalledWith(
                "order-1",
                { paymentStatus: "paid", transactionId: "txn-123", paidAt: expect.any(Date) },
                { new: true }
            );
            expect(result.paymentStatus).toBe("paid");
        });
        ...
        it("should update payment status to failed", async () => {
            const mockOrder = { _id: "order-1", paymentStatus: "failed" };

            (OrderModel.findByIdAndUpdate as jest.Mock).mockResolvedValue(mockOrder);

            const result = await orderService.updatePaymentStatus("order-1", "failed");
